import { ImageStyle, StyleProp, TouchableOpacity, ViewStyle } from "react-native"
...
export function UserItem({style,pictureContainerStyle,picture,name,color,onPress}:Props) {
    return(
        <TouchableOpacity
        onPress={onPress}        
      >
        <View center marginR-25 style={style}>
          <AnimatedImage
            ...
            borderRadius={64} 
          />
          <Text color={color} text70R>{name}</Text>
        </View>
      </TouchableOpacity>
    )
}