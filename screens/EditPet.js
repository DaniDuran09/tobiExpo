...
const EditPet = ({ route, navigation }) => {
    ...
    const onSubmit = () => {
        ...
        if (user.name === null || user.last_name === null || user.age === null || user.weight === null) {
        ...
        <KeyboardAvoidingView style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', }} behavior={'height'} enabled  >
            <ScrollView>
                <View style={{
                    ...
                    <View style={{ height: '20%', width: '100%', justifyContent: 'flex-start', alignItems: 'center', flexDirection: 'row', paddingHorizontal: 25 }}>
                        <TouchableWithoutFeedback onPress={() => { setActiveModal(true), setCameraType('front') }}>
                            <View>
                                <Image source={{ uri: picture === 0 ? petInfo.picture : imageSourceFront }} style={{ height: 110, width: 110, borderRadius: 100, zIndex: 0 }} resizeMode={'contain'} />
                                <View style={{ height: 110, width: 110, backgroundColor: 'rgba(0,0,0,0.3)', zIndex: 1, position: 'absolute', borderRadius: 100, justifyContent: 'flex-end', alignItems: 'flex-end', paddingBottom: 15, paddingRight: 15 }}>
                                    <View style={{ backgroundColor: 'rgba(0,0,0,0.7)', padding: 7, borderRadius: 100 }}>
                                        <Image source={require('../assets/camW.png')} style={{ height: 25, width: 25 }} resizeMode='contain' />
                                    </View>
                                </View>
                            </View>
                        </TouchableWithoutFeedback>
                        <Text style={{ fontSize: 18, fontWeight: 'bold', textAlign: 'center', color: '#EF4136', marginLeft: 20 }}>Edita Tu Mascosta</Text>
                    </View>
                    <View style={{ height: '60%', width: '100%', justifyContent: 'space-around', alignItems: 'center' }}>
                        <TextInput
                            placeholder="Nombre"
                        ...
                        <TouchableWithoutFeedback onPress={() => setOpen(true)}>
                            <View elevation={5} style={{ width: '90%', backgroundColor: '#D6EFFF', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: '5%', borderRadius: 10 }}>
                                <DatePicker
                                    ...
                                    title={'Cumpleaños'}
                                />
                                <Text style={{ textAlign: 'center', fontSize: 14, color: 'black', fontWeight: '400', paddingVertical: 10 }}>{user.birthday === '' ? petInfo.birthday : `${user?.birthday.toLocaleDateString('es-us')}`}</Text>
                                <Image source={require('../assets/pastel.png')} style={{ height: 30, width: 30 }} resizeMode={'contain'} />
                            </View>
                        </TouchableWithoutFeedback>
                    </View>
                    <View style={{ height: '20%', width: '100%', justifyContent: 'flex-start', alignItems: 'center' }}>
                        <TouchableWithoutFeedback onPress={onSubmit}>
                            <View style={{ height: 50, width: '80%', backgroundColor: '#EF4136', borderRadius: 10, justifyContent: 'center', marginBottom: 15 }}>
                                <Text style={{ textAlign: 'center', fontSize: 16, color: 'white', fontWeight: '700' }}>Guardar</Text>
                            </View>
                        </TouchableWithoutFeedback>
                    </View>
                </View>
